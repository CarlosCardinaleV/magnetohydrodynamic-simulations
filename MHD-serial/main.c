...
/**
 * @brief Esta función realiza una simulación de un sistema
 * Magnetohidrodinámico (MHD).
 * Inicializa dos regiones con diferentes conjuntos de propiedades
 * físicas (densidad, velocidad, presión, campos magnéticos), 
 * y calcula el estado en puntos intermedios utilizando una función
 * de transición suave (`tanh`). Luego, genera los primeros tres pasos
 * de tiempo de la simulación y entra en un bucle que avanza la solución
 * en el tiempo hasta que se alcanza el tiempo máximo de simulación.
 * En cada paso de este bucle, calcula el siguiente paso de tiempo potencial,
 * lo corrige hasta que la corrección sea menor que un cierto umbral, 
 * y desplaza los pasos de tiempo anteriores hacia atrás, descartando 
 * el más antiguo y utilizando el recién calculado como el más reciente.
 * Finalmente, libera toda la memoria que había asignado durante la simulación.
 *
 * @return int - Siempre devuelve 0 indicando que la simulación se ha
 *         realizado correctamente.
 *
 * @note El resultado de la simulación se almacena en un archivo llamado
 *         "datos.txt". Este archivo se puede utilizar para graficar los resultados
 *          con GNUPlot.
 * 
 * @warning Asegúrese de tener suficiente memoria para realizar la simulación. 
 * Si se alcanza el límite de la memoria, la función puede causar un error de 
 * segmentación. Esto sucede ya que el programa terminaria bruscamente y tambien
 * habria memoria no finalizada por los exit(), en le caso que calloc() falle.
*/
int mhd(void);
 ...
 *    F[nNodos..2*nNodos-1] = By*Vx - Bx*Vy,
 *    F[2*nNodos..3*nNodos-1] = Bz*Vx - Bx*Vz,
 *    F[3*nNodos..4*nNodos-1] = rho*Vx,
 *    F[4*nNodos..5*nNodos-1] = rho*Vx^2 + p + B^2/2 - Bx^2,
 *    F[5*nNodos..6*nNodos-1] = rho*Vx*Vy - Bx*By,
 *    F[6*nNodos..7*nNodos-1] = rho*Vx*Vz - Bx*Bz,
 *    F[7*nNodos..8*nNodos-1] =
...
int main(int argc, char* argv[]) {
    ...
    if ( mhd() != EXIT_SUCCESS) {
        fprintf(stderr, "Eror: program failed!\n");
    }
...
int mhd(void) {
    int nNodos = NODOS; double W = 0.0085;
    double* x = linspace(-1, 1, nNodos); double dt = 0.0005;
    double tMax = 0.20;

    double rho1 = 1.0; double Vx1 = 0.0;
    double Vy1 = 0.0; double Vz1 = 0.0;
    double p1 = 1.0; double Bx1 = 0.75;
    double By1 = 1.0; double Bz1 = 0.0;

    double rho2 = 0.125; double Vx2 = 0;
    double Vy2 = 0; double Vz2 = 0;
    double p2 = 0.1; double Bx2 = 0.75;
    double By2 = 0.75; double Bz2 = 0.75;
    ...
    //printTresU(resultado);
    //printf("\n");
    double* LUnMenos3 = getLU(UNMenos3);
    double* LUnMenos2 = getLU(UNMenos2);
    double* LUnMenos1 = getLU(UNMenos1);
    //printMatrix(LUnMenos1, 8, nNodos);
    //printf("\n");
    //printMatrix(LUnMenos2, 8, nNodos);
    //printf("\n");
    //printMatrix(LUnMenos3, 8, nNodos);
...
double* getF(double* U) {
    ...
    double* F = (double*) calloc(nNodos * 8, sizeof(double));
